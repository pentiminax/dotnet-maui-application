...
{
    const string BASE_URL = "https://world.openfoodfacts.org";
    const string BASE_SEARCH_URL = "https://fr.openfoodfacts.org/cgi/search.pl";
    ...
    public async Task<List<Product>> GetRandomProductsAsync()
    {
        ...
        var url = $"{BASE_URL}?json=true&page={page}&page_size=10&{ProductService.GetNutriScoreFilter(0)}";
    ...
    public async Task<List<Product>> SearchProductsAsync(string searchTerm)
    {
        ...
        var url = $"{BASE_SEARCH_URL}?action=process&json=true" +
